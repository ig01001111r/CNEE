...
fprintf('\nVetor solução do sistema triangular inferior (y):\n'); disp(transposta(y));
fprintf('\nVetor solução do sistema triangular superior (x):\n'); disp(transposta(x));
...
if possivelSeDominante
    ...
    fprintf("\nexiste uma permutação que torna a matriz DIAGONALMENTE DOMINANTE!\n");
    P = matrizPermutacao;  % Renomeia para ficar coerente
    A_permutada = P * A;
    fprintf("\nMatriz A permutada:\n"); disp(A_permutada);
    ...
    toler = 1e-5;
    iterMax = 30;
    [x_it, iter, ~] = gauss_seidel(A, b, toler, iterMax);
...
% Cálculo da inversa via Cholesky
inv_chol = inv_cholesky(A);
fprintf("\nMatriz inversa de A:\n"); disp(inv_chol);
